...
const RegisterBuyerPerusahaan = () => {
  const URLLink = 'http://localhost:8000';
  const { error, isPending, data: badanusahas } = useFetch(URLLink+'/api/badanusaha');
  const { error :error2, isPending: isPending2, data: klasifikasiperusahaans } = useFetch(URLLink+'/api/klasifikasiperusahaan');
  const { error: error3, isPending: isPending3, data: provinsis } = useFetch(URLLink+'/api/provinsi');
  ...
  const [klasifikasiperusahaan, setKlasifikasiPerusahaan] = useState('');
  const [errKlasifikasiPerusahaan, setErrKlasifikasiPerusahaan] = useState(null);
  ...
  const handleSubmit = async (e) => {
    ...
    if (!klasifikasiperusahaan) {
      setErrKlasifikasiPerusahaan("Klasifikasi usaha harus dipilih");
      submitted = false;
    }
    ...
    if (!fileDp) {
      setErrFileDp("File domilisi perusahaan harus dilengkapi");
      submitted = false;
    }
            ...
            <div className="form-tab">
              <ul className="nav nav-pills nav-fill" role="tablist">
                <li className="nav-item">
                    <a className="nav-link active" id="register-tab-2" data-toggle="tab" href="#register-2" role="tab" aria-controls="register-2" aria-selected="true">Pendaftaran Pelanggan Perusahaan</a>
                </li>
              </ul>
                ...
                <div className="tab-pane fade show active" id="register-2" role="tabpanel" aria-labelledby="register-tab-2">
                  <form ref={form} onSubmit={handleSubmit}>
                    ...
                    <div className="form-group">
                      ...
                      <div className="select-custom">
                        <select name="klasifikasiperusahaan" id="klasifikasiperusahaan" className={`form-control ${errBadanUsaha ? 'is-invalid' : ''}`} defaultValue={klasifikasiperusahaan} onChange={(e) => { setKlasifikasiPerusahaan(e.target.value); e.target.value && setErrKlasifikasiPerusahaan(null) }}>
                          <option value="">{isPending2 ? 'Loading....' : 'Pilih klasifikasi usaha'}</option>
                            { klasifikasiperusahaans && klasifikasiperusahaans.map(klasifikasiperusahaan => {
                              return(
                                <option key={klasifikasiperusahaan.iId} value={klasifikasiperusahaan.iId}> {klasifikasiperusahaan.vNama}</option>
                              )
                            })}
                        </select>
                        <div className="invalid-feedback">{errKlasifikasiPerusahaan}</div>
                      </div>
                    </div>
                      ...
                      <div className="form-group col-lg-6">
                        ...
                        <div className="select-custom">
                            <select id="provinsi" name="provinsi" className={`form-control ${errProvinsi ? 'is-invalid' : ''}`} defaultValue={provinsi} onChange={changeProvinsi} >
                                <option value="">{isPending3 ? 'Loading....' : 'Pilih Provinsi'}</option>
                    ...
                    <div className="form-group">
                      <label>No.SIUP *</label>
                      <input type="text" className={`form-control ${errNoSiup ? 'is-invalid' : ''}`} id="siup" name="siup" placeholder="Masukkan no.siup" value={noSiup} onChange={(e) => { setNoSiup(e.target.value); e.target.value && setErrNoSiup(null) }} />
                      <div className="invalid-feedback">{errNoSiup}</div>
                    </div>

                    <label>File SIUP *</label>
                    <div className="form-group custom-file">
                      <label className="custom-file-label">{fileSiup ? fileSiup.name : 'Pilih file isup'}</label>
                    ...
                    <div className="form-group custom-file">
                      ...
                      <input type="file" className={`custom-file-input ${errFileAp ? 'is-invalid' : ''}`} id="fileaktapendirian" name="fileaktapendirian" onChange={changeFileap} />
                    ...
                    <div className="form-group custom-file">
                      ...
                      <input type="file" className={`custom-file-input ${errFileDp ? 'is-invalid' : ''}`} id="filedomisiliperusahaan" name="filedomisiliperusahaan" onChange={changeFiledp} />
...
export default RegisterBuyerPerusahaan;
